...
path = 'C:/Users/faizz/University Work/Year 4/Advanced Project/Dataset/test';
d = dir(fullfile(path, '*.pgm'));
c = struct;
F = zeros(686, 1);

for k = 1 : length(d)
 baseFileName = d(k).name;
 fullFileName = fullfile(d(k).folder, baseFileName);
 fprintf(1, 'Now reading %s\n', fullFileName);
 F = spam_extract_2(double(imread(fullFileName)),3,k, F);
end

function F = spam_extract_2(X,T,k, F)
